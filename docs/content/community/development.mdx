...
```bash
./bracket/backend$ docker build -t bracket-vko-app .
./bracket/backend$ docker tag bracket-vko-app kozmin83/bracket-vko-app:latest
./bracket/backend$ docker push kozmin83/bracket-vko-app:latest
```

## Build and deploy frontend app
```bash
./bracket/frontend$ pnpm build
./bracket/frontend$ vercel --prod
```
